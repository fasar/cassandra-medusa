...
def write_config(name, encrypted, prefix):
    lines = [
        '[cassandra]',
        'is_ccm = 1',
        'stop_cmd = ccm stop',
...
def main():
    ...
    plain_cfg = write_config('plain', encrypted=False, prefix='bench_plain')
    cse_cfg = write_config('cse', encrypted=True, prefix='bench_cse')
    ...
    for label, cfg, name in (('M1', plain_cfg, 'plain'), ('M2', cse_cfg, 'cse')):
        ...
        for i in range(args.repeats):
            ...
            shutil.rmtree(WORK / 'bucket' / f'bench_{name}', ignore_errors=True)
            wait_for_compactions()
        ...
        results[f'S_{label}'] = stored_bytes(f'bench_{name}')
        results[f'F_{label}'] = stored_files(f'bench_{name}')
...
def report(r, dataset, args):
    ...
    print('\n' + '=' * 76)
    print('T1  dataset D = {:,} bytes ({:.2f} GiB), median of {} runs'.format(
        dataset, gib, args.repeats))
    print('=' * 76)
