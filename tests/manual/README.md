...
Client-side encryption is implemented with the Amazon S3 Encryption Client, so it exists for the S3
providers only: the two configurations share the storage backend and differ by encryption alone.
The local provider appears in one check, A10, which verifies that a key configured with it is
refused.

| # | Configuration | Storage provider | `key_secret_base64` |
|---|---|---|---|
| C1 | `minio-plain` | s3_compatible | absent |
| C2 | `minio-cse` | s3_compatible | set |
...
| ID | Step | Pass criterion |
|---|---|---|
| **A7** | `head_object` on an SSTable object, and the sizes in `manifest.json` | **CSE on**: the object carries the S3 Encryption Client metadata (`x-amz-meta-x-amz-3`, the wrapped data key) and every object is exactly 16 bytes (the GCM tag) longer than its file · **CSE off**: no such metadata |
...
| **A10** | `medusa list-backups` with `storage_provider = local` and a key | exit code ≠ 0 and a message saying encryption is S3-only |

A7 is what actually proves the data is encrypted. Everything else could pass with encryption silently
disabled. A10 runs once, before the configurations.
...
A configuration passes when every step A1-A9 and R1-R3 meets its criterion, and A10 must pass on its own. Any row count that
...
Prerequisites: CCM with Cassandra 4.1.9, `JAVA_HOME` set to a JDK 11, Medusa installed with the
`encryption` extra (`poetry install --extras encryption`), and a MinIO reachable at `127.0.0.1:9000`
with credentials in `~/.aws/minio_credentials`.

```bash
export JAVA_HOME=/usr/lib/jvm/java-11-openjdk
ulimit -n 8192
cd tests/manual && ./run_manual_cse_tests.sh            # both configurations
./run_manual_cse_tests.sh minio-cse                     # or just one
```
