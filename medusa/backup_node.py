...
def make_manifest_object(fqdn, snapshot_path, manifest_objects, storage):
    return {
        ...
        'objects': [{
            ...
            **({'remote_md5': getattr(manifest_object, 'remote_md5')} if getattr(manifest_object, 'remote_md5', None) is not None else {}),
            **({'remote_size': getattr(manifest_object, 'remote_size')} if getattr(manifest_object, 'remote_size', None) is not None else {}),
