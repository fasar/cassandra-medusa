...
| `encryption_frame_length` (default 8388608) | |
...
# GCS
gsutil du -s gs://<bucket>/bench_plain
gsutil du -s gs://<bucket>/bench_cse

# Azure
az storage blob list -c <container> --prefix bench_plain --query "sum([].properties.contentLength)"
az storage blob list -c <container> --prefix bench_cse   --query "sum([].properties.contentLength)"
```
...
Expected to be small and roughly constant per file: the AWS Encryption SDK adds a header plus one
authentication tag per frame. **Beware**: with many small SSTable components the relative overhead
climbs, so quote C5 alongside the file count.
